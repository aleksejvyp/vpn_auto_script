...
3. После первого запуска и нажатия сочетания клавиш для включения VPN в папке появится файл `config.json`. В нем необходимо заполнить две строки:
   - Имя окна вашего приложения (очень важно, без него скрипт не будет работать).
   - Путь до exe файла используемого вами VPN клиента. Путь пишется в том же формате, что и в config, через двойной слэш. Если путь есть, вы можете запускать приложение с помощью скрипта и сразу его включать. 
